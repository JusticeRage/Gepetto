...
def comment_callback(address, view, response, start_time):
    ...
    try:
        ida_kernwin.execute_sync(_apply_comment, ida_kernwin.MFF_WRITE)
    except Exception:
        # Fall back to best-effort without UI guarantees
        _apply_comment()
...
class GenerateCCodeHandler(idaapi.action_handler_t):
    ...
    def _save_c_code(self, view, response):
        ...
        def _gather_and_refresh():
            ...
            return 1
        try:
            ida_kernwin.execute_sync(_gather_and_refresh, ida_kernwin.MFF_FAST)
        except Exception:
            _gather_and_refresh()
