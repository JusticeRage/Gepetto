...
def comment_callback(address, view, response, start_time):
    ...
    comment = re.sub(
        r"----- "
        + tr_("Comment generated by Gepetto")
        + " -----.*?----------------------------------------",
        r"",
        comment,
        flags=re.DOTALL,
    ...
    idc.set_func_cmt(
        address,
        "----- " + tr_("Comment generated by Gepetto") + f" -----\n\n"
        f"{response.strip()}\n\n"
        f"----------------------------------------\n\n"
        f"{comment.strip()}",
        0,
    ...
    print(
        tr_("{model} query finished in {time:.2f} seconds!").format(
...
class ExplainHandler(idaapi.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        gepetto.config.model.query_model_async(
            tr_(
                "Can you explain what the following C function does and suggest a better name for "
                "it?\n{decompiler_output}"
...
def rename_callback(address, view, response):
    ...
    print(
        tr_("{model} query finished! {replaced} variable(s) renamed.").format(
            model=str(gepetto.config.model), replaced=len(replaced)
        )
    )
...
class SwapModelHandler(idaapi.action_handler_t):
    ...
    def activate(self, ctx):
        try:
            ...
            ValueError
        ) as e:  # Raised if an API key is missing. In which case, don't switch.
            print(
                tr_("Couldn't change model to {model}: {error}").format(
                    model=self.new_model, error=str(e)
                )
            )
            return
...
class GenerateCCodeHandler(idaapi.action_handler_t):
    ...
    def _save_c_code(self, view, response):
        ...
        if view:
            view.refresh_view(False)
        print(
            tr_("{model} generated code saved to {file_name}").format(
