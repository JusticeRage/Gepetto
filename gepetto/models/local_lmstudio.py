...
class LMStudio(GPT):
    ...
    def supported_models() -> list:
        ...
        base_url = gepetto.config.get_config(
            "LMStudio", "BASE_URL", default="http://127.0.0.1:1234/v1/"
        )
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    tr_("Failed to fetch models from {base_url}: {status_code}").format(
    ...
    def query_model(self, query, cb, additional_model_options=None):
        if (
            additional_model_options is not None
            and additional_model_options.get("response_format", {}).get("type")
            == "json_object"
        ):
